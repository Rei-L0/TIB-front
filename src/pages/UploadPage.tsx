          ...
          <div className="bg-emerald-50 rounded-2xl p-4 mb-4">
            ...
            {weatherInfo && (
              <div className="mt-3 p-3 bg-white rounded-xl">
                <p className="text-xs text-gray-500 mb-2">🌤 촬영일 날씨 ({weatherInfo.station})</p>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    <p className="text-lg font-bold text-gray-900">{weatherInfo.avgTemp}°C</p>
                    <p className="text-xs text-gray-500">평균</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-red-500">{weatherInfo.maxTemp}°C</p>
                    <p className="text-xs text-gray-500">최고</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-blue-500">{weatherInfo.minTemp}°C</p>
                    <p className="text-xs text-gray-500">최저</p>
                  </div>
                </div>
                <div className="flex justify-between mt-2 text-sm text-gray-600">
                  <span>💧 습도 {weatherInfo.humidity}%</span>
                  <span>☀️ 일조 {weatherInfo.sunshine}h</span>
                  {weatherInfo.rainfall > 0 && <span>🌧 강수 {weatherInfo.rainfall}mm</span>}
                </div>
              </div>
            )}
          </div>
